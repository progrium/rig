...
import (
	...
	inspectorweb "github.com/progrium/rig/pkg/inspector/web"
...
func (m *Server) Activate(ctx context.Context) (err error) {
	if m.srv != nil {
		return nil
	}
	...
	m.srv = &http.Server{
		...
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			h := http.NewServeMux()
			h.Handle("/-/inspector/", http.StripPrefix("/-/inspector", http.FileServerFS(inspectorweb.Dir)))
			h.HandleFunc("/-/inspector/ws", func(w http.ResponseWriter, r *http.Request) {
				if !gwebsocket.IsWebSocketUpgrade(r) {
					http.Redirect(w, r, "/-/inspector/inspector.html", http.StatusFound)
					return
				}
				websocket.Handler(func(ws *websocket.Conn) {
					...
					peer.Handle("setValue", rpc.HandlerFunc(func(r rpc.Responder, c *rpc.Call) {
						...
						if err := telepath.Select(entity.Value(n.Entity()), parts[1]).Set(action.Value); err != nil {
							r.Return(err)
							return
						}
						entity.Send(n, "UpdateValue", action.Selector, action.Value)
					}))
	...
	store := m.Object().Store().(*node.Store)
	store.Watch(m)

	return nil
}

func (m *Server) Signaled(s signal.Signal[entity.E]) {
	n := node.Snapshot(s.Receiver.(entity.E))
	m.updates.Publish(n)
}

func (m *Server) handlePeer(peer *talk.Peer) {
...
func (m *Server) Deactivate(ctx context.Context) error {
	if m.srv != nil {
		srv := m.srv
		m.srv = nil
