...
import (
	...
	"github.com/progrium/rig/pkg/shlex"
...
func (c *Conn) ResolveFS(ctx context.Context, name string) (fs.FS, string, error) {
	fsys := fskit.MapFS{
		...
		"status": FieldFile(func() (string, error) { return c.status(), nil }),
		"local":  FieldFile(func() (string, error) { return c.local(), nil }),
		"remote": FieldFile(func() (string, error) { return c.remote(), nil }),
...
// below pulled from wanix internal, and should probably not be internal
// so that we can remove these here
